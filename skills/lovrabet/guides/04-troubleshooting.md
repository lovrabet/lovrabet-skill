...
* MCP 工具调用失败，提示“Dataset not found”
...
**AI 的第一反应**：立刻调用 `get_dataset_detail` 核对 Schema。

* ❌ 错误排查方向：按自己的猜测随便换一个字段名重试
* ✅ **强制动作**：
  1. 调用 `get_dataset_detail` 获取真实数据集信息
  2. 提取 `basic.tableName` 作为表名
  3. 逐一核查报错字段是否存在于 `fields` 列表中，注意大小写
  4. 确认无误后再重新生成 SQL 并通过 `validate_sql_content` 验证
...
### 5. 听到“MCP 报错 / Dataset not found”
**AI 的第一反应**：检查基础连通性，指导用户确认环境。

* ❌ 错误排查方向：建议用户重装各种库
* ✅ **强制动作**：
  * 建议用户检查 `.lovrabetrc`（或其他配置）中的 `appcode` 是否正确
  * 建议用户在终端执行 `lovrabet auth` 检查是否未登录或 Cookie 过期
  * 帮用户调用 `list_datasets` 确认该环境到底有哪些可用数据集

---

## 核心原则

* **不要猜**：只要涉及“不存在”、“不匹配”，第一步永远是通过 MCP 工具（如 `get_dataset_detail`）拉取最新真实数据。
