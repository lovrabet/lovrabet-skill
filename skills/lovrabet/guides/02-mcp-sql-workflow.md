...
> 目标：指导 AI 如何正确调用 MCP 工具来完成 SQL 的查询、验证、保存与测试，并提供平台支持的 MyBatis 动态 SQL 语法参考。
>
> 前置阅读：`07-sql-creation-workflow.md`（整体流程）、`06-data-api-guidelines.md`（字段约束）

## 何时使用

当任务满足任一条件时，必须阅读并遵守本指南：

* 需要调用 MCP 工具处理自定义 SQL
* 编写需要动态条件的复杂 SQL（如可选参数、范围过滤）
* SQL 验证报错，需要排查语法或字段问题

## MCP 工具调用规范

在执行 `07-sql-creation-workflow.md` 规定的流程时，AI 必须严格按以下方式调用 MCP 工具，**不要编写额外的 TS 脚本来调用，直接使用环境内置的工具调用能力**。
...
* **用途**：保存前的强制卡点。
* **参数**：传入本地 `.sql` 文件的内容和 `dbId`。
* **动作**：如果 `valid: false`，必须根据 `errors` 提示修改本地 SQL，然后重新调用本工具，直到通过。

### 4. `save_or_update_custom_sql`
* **用途**：将验证通过的 SELECT 语句保存到平台。
* **限制**：INSERT / UPDATE / DELETE / DDL 等会引发异常或被拦截，应指导用户保存在本地。
* **冲突处理**：如果返回 `blocked: true`，说明有作者冲突或拦截，**严禁重试**，直接告知开发者。

### 5. `execute_custom_sql`
* **用途**：保存成功后的功能测试。
* **动作**：测试失败（如语法错、数据不符合预期）时，必须回到本地修改 -> 重新 validate -> 重新 save -> 重新 execute，形成闭环。
...
* ❌ 禁止在未调用 `validate_sql_content` 通过前，直接调 `save_or_update`
