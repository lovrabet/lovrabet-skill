...
其中 `datasetId` 需要通过 `get_dataset_detail` 获取。
...
写任何 BFF 逻辑前，必须调用 `get_dataset_detail`，必要时先用 `list_datasets` 或 `search_datasets` 定位数据集。
...
编写 BFF 前，必须阅读 `06-data-api-guidelines.md` 中的性能优化部分。
