...
### Step 1：模型与关系推演
- **强制动作**：使用 MCP（`list_datasets`、`search_datasets`、`get_dataset_detail`）找出所有相关的业务模型。
...
* 高危 DDL/DML 不应通过 MCP 自动保存到平台，必须建议用户保存在本地 `src/custom_sql/` 进行 review。
...
在通过 MCP 保存资源（如 SQL、BFF）时，应善用 `description` 字段。AI 应在描述中主动加入语义化的标签和版本说明。
...
当遇到 `save_or_update` 返回 `blocked` 或提示有冲突时：
