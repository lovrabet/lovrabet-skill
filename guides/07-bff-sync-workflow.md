...
>
> **前置阅读**：`05-backend-function` - Backend Function 脚本编写规范
...
该标志与 `lovrabet skill install --enable-bff-save` 使用**同一名称**（与 CLI 动态 flag 对齐）；旧名 `--dangerously-bff-save` 仍可用。配置后重启 MCP 客户端或重载 MCP。
...
技能包会**全量安装**所有指南；未加该参数时仍可阅读本指南，但需按上述方式开启 MCP 后才能用工具保存 BFF。
...
- **Backend Function 编写规范**：`.lovrabet/skill/.shared/guides/05-backend-function/guide.md`
- **数据接口访问规范**：`.lovrabet/skill/.shared/guides/06-data-api-guidelines/guide.md`
